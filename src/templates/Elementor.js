...
const ElementorTemplate = ({ pageContext }) => {
    ...
    const Page = elementorData.map(row => {
      ...
      return (
        <Section key={row.id} settings={row.settings} className={ row.settings?.css_classes || null}>
          <Container className={row.settings.layout === 'full_width' ? 'px-0' : ''} fluid={row.settings.layout === 'full_width' ? 'xl' : ''}>
            <Row className="w-100 px-0 mx-0">
              {
                row.elements.map(column => {
                  ...
                  return (
                    <Col className={column?.settings?.css_classes || ''} style={ColStyle} key={column.id} xs={bsCOLgen(column?.settings?._inline_size_mobile || 100)} sm={bsCOLgen(column?.settings?._inline_size_tablet || 100)} md={bsCOLgen( column?.settings?._inline_size || column?.settings?._column_size)}>
                      {
                        column.elements.map(widget => {
                          const elTypeSection = widget.elType === 'section';
                          //console.log(widget); 
                          if (elTypeSection) {
                            return <InnerSection key={widget.id} InnerSectionData={widget} />
                          } else if (!elTypeSection){
                            return (
                              React.createElement(
    ...
    return (
      <Layout seo={pageContext.post.seo}>
        {Page}
      </Layout>
    )
}
